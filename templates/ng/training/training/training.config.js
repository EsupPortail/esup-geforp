...
sygeforApp.config(["$trainingBundleProvider", "$listStateProvider", "$dialogProvider", function ($trainingBundleProvider, $listStateProvider, $dialogProvider) {
    ...
    $listStateProvider.state('training', {
        ...
        breadcrumb: function ($stateParams, $trainingBundle) {
            ...
            if ($stateParams.type) {
                breadcrumb.push({
                    label: $trainingBundle.getType($stateParams.type).label,
                    sref: "training.table({type: '" + $stateParams.type + "'})"
                });
            }
            return breadcrumb;
        },
                ...
                states: {
                    view: {
                        ...
                        breadcrumb: function ($stateParams, data, $trainingBundle) {
                            ...
                            if (!$stateParams.type) {
                                breadcrumb.push({
                                    label: $trainingBundle.getType(data.training.type).label,
                                    sref: "training.table({type: data.training.type})"
                                });
                            }
